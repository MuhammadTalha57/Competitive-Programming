...
void solve() {
    ...
    vvi dp(n + 1, vi(m + 1, 0));
    // dp[i][j] = Min cost for matching
    // first i characters of s with first j characters of t
    ...
    rep(i, 1, (n + 1)) {
        rep(j, 1, (m + 1)) {
            ...
            // 3. Replace s[i - 1] with t[j - 1]
            // Cost = (s[i - 1] != t[j - 1]) + dp[i - 1][j - 1]

            dp[i][j] = min( 1 + dp[i - 1][j], min(1 + dp[i][j - 1], (s[i - 1] != t[j - 1]) + dp[i - 1][ j - 1]) );
