...
void build(vi& arr, int u, int l, int r) {
    if(l == r) {
        ...
        if(segTree[u * 2].first == segTree[u * 2 + 1].first) {
            segTree[u] = segTree[u * 2];
        } else {
...
pii query(int u, int l, int r, int tl, int tr) {
    if(tl > tr) return {-1, -1};
    if(tl == l && tr == r) {
        return segTree[u];
    } else {
        int m = (l + r) / 2;
        pii left = query(u * 2, l, m, tl, min(m, tr));
        pii right = query(u * 2 + 1, m + 1, r, max(m + 1, tl), tr);
...
void solve() {
    ...
    while(q--) {
        ...
        // print(query(1, 1, n, a, b).first, query(1, 1, n, a, b).second);
        print(temp[a] - temp[ query(1, 1, n, a, b).second ] + 1);
    }
