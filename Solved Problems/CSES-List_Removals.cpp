...
void build(vi& arr, int u, int l, int r) {
    if(l == r) {
        ...
        segTree[u] = {segTree[u * 2].first + segTree[u * 2 + 1].first, r};
    }
    // cout << l << ' ' << r << ' ' << segTree[u].first << '\n';
}
...
void update(int u, int l, int r, int k) {
    ...
    if(k == l && k == r) {
        ...
        int m = (l + r) / 2;
        update(u * 2, l, m, k);
        update(u * 2 + 1, m + 1, r, k);
        segTree[u] = {segTree[u * 2].first + segTree[u * 2 + 1].first, max(segTree[u * 2].second, segTree[u * 2 + 1].second)};
    }
}

int query(int u, int l, int r, int p) {
    if(p > segTree[u].first) return 0;

    // cout << l << ' ' << r << ' ' << segTree[u].first << ' ' << segTree[u].second << ' ' << p << '\n';
    if(segTree[u].first == p) return segTree[u].second;

    int m = (l + r) / 2;
    if(segTree[u * 2].first >= p) {
        return query(u * 2, l, m, p);
    }
    return query(u * 2 + 1, m + 1, r, p - segTree[u * 2].first);
}
