...
void solve() {
    ...
    vpll dp((1 << n));
    // dp[x] = {minRides, lastRideWeight}
    // dp[x] consider all persons for which bit is set

    // Base case
    // No person selected, Ride needed = 1, Weight = 0
    dp[0] = {1, 0};

    // Iterate over masks
    for(int subset = 1; subset < (1 << n); subset++) {
        dp[subset] = {21, 0}; // Max Rides cuz n <= 20
