...
int dfs(int u, int p, vvi& adj, vi& child, vi& par) {
    par[u] = p;
    for(int v: adj[u]) {
        if(v == p) continue;
        child[u] += 1 +  dfs(v, u, adj, child, par);
    }
    return child[u];
}

int n, k;
void dfs2(int u, int p, vvi& adj, vi& child, ll orgCnt, ll& ans) {
    ans += orgCnt;
    // cout << u << ' ' << cnt << '\n';
    for(int v: adj[u]) {
        if (v == p) continue;
        ...
        if(n - 1 >= k - 1) {
            // Par was included
            if(n - 1 - (1 + child[v]) < k - 1) cnt--;
        }

        if(child[v] < k - 1) {
            if(n - 1 >= k - 1) cnt++;
        }
        dfs2(v, u, adj, child, cnt, ans);
    }
}
