...
int n;
ll dfs(vvll& mat, int i, int j, map<pii, ll>& cache) {
    if(i >= n || j >= mat[i].size()) return 0;
    ...
    ll res = mat[i][j];
    res += max(dfs(mat, i + 1, j, cache), dfs(mat, i + 1, j + 1, cache));
...
void solve() {
    ...
    print(dfs(mat, 0, 0, cache));
}

// Problem Link: https://vjudge.net/problem/CodeChef-SUMTRIAN/origin
