...
void push(int u) {
    if(marked[u] != 0) {
        segTree[u * 2] += marked[u];
        segTree[u * 2 + 1] += marked[u];
        marked[u * 2] += marked[u];
        marked[u * 2 + 1] += marked[u];
...
ll query(int u, int l, int r, int tl, int tr) {
    ...
    push(u);
    int m = (l + r) / 2;
    return query(u *2, l, m, tl, min(m, tr)) + query(u * 2 + 1, m + 1, r, max(m + 1, tl), tr);
}
