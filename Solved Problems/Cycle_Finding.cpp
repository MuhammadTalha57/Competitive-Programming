...
void dfs(int u, vector<vector<pll>>& adjList, vi& vis, int cid) {
    vis[u] = cid;

    for(auto [v, w]: adjList[u]) {
        if(vis[v] == 0) {
            dfs(v, adjList, vis, cid);
        }
    }
}


bool func(int u, vector<vector<pll>>& adjList, vi& vis, int cid) {
    vll dist(n + 1, INF);
    dist[u] = 0;
    vi par(n + 1, 0);
    vi cycle;
    ...
    rep(i, 0, n) {
        rep(u, 1, (n + 1)) {
            if(vis[u] != cid || dist[u] == INF) {continue;}
            for(auto [v, w]: adjList[u]) {
                if(dist[v] > dist[u] + w) {
                    par[v] = u;
                    if(i == n - 1) {
...
void solve() {
    ...
    rep(u, 1, (n + 1)) {
        if(vis[u] == 0) {
            dfs(u, adjList, vis, ++cid);
            if(func(u, adjList, vis, cid)) {
                return;
            }
        }
    }
