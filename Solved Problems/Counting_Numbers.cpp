...
ll dp(string temp, int i) {
    if(i == temp.size()) {return 0;}
    ll totalDigs = temp.size();
    ll res = 0;
    if(temp[i] != '0') {
        res += static_cast<ll>( ( temp[i] - '0' ) - 1) * ninePow[totalDigs - i]; 
    }

    return res + dp(temp, i + 1);
...
ll calc(ll n) {
    ...
    // return the number of required numbers in the range [0, n]
    string temp = to_string(n);

    ll res = 0;

    ll firstDig = temp[0] - '0';
    ll totalDigs = temp.size();

    // Nums whose length < totalDigs
    for(int i = 1; i < totalDigs; i++) {
        res += ninePow[i];
    }   
