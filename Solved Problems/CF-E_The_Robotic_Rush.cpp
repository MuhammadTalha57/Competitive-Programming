...
void solve() {
    ...
    rep(k, 0, n) {
        while(i != m - 1 && b[i + 1] <= arr[k]) {i++;}

        if(b[i] <= arr[k]) {
            left.insert({arr[k] - b[i], k});
        }

        while(j != m && b[j] <= arr[k]) {j++;}

        if(j != m) {
            right.insert({b[j] - arr[k], k});
        }
    }
    ...
    for(char c: s) {
        ...
        while(!left.empty() && (*left.begin()).first <= -off) {
            if( alive[ (*left.begin()).second] ) {
                alive[ (*left.begin()).second] = 0;
                ans--;
            }
        ...
        // Right
        while(!right.empty() && (*right.begin()).first <= off) {
            if( alive[ (*right.begin()).second] ) {
                alive[ (*right.begin()).second] = 0;
                ans--;
            }
