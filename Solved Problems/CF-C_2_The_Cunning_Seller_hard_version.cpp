...
void solve() {
    ...
    vll powCount(temp.size());

    for(int x = p -1; x >= 0 && n > 0; x--) {
        ll div = n / temp[x];
        res += div * ( (temp[x]*(9 + x)) / 3 );
        n = n % temp[x];
        count += div;
        powCount[x] = div;
    }
    ...
    ll rem = k - count;
    for(int x =  p - 1; x > 0; x--) {
        ll extra = min(rem / 2, powCount[x]);

        powCount[x] -= extra;
        powCount[x - 1] += (3*extra);
        rem -= 2*extra;
        debug(x, extra, rem, res);
        res -= (extra * ( (temp[x]*(9 + x)) / 3 ));
        res += 3*extra*( (temp[x - 1]*(9 + x - 1)) / 3 );
...
// Problem Link: https://codeforces.com/contest/2132/problem/C2
