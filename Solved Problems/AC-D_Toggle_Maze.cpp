...
void solve() {
    ...
    vector<vector<vi>> vis = vector<vector<vi>>(n, vector<vi>(m, vi(2, 0)));

    queue<vi> q;
    q.push({x, y, 0});
    vis[x][y][0] = 1;

    vector<pii> neigh = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    ...
    while(!q.empty()) {
        ...
        for(auto [dx, dy]: neigh) {
            ...
            if(g[r][c] == '.' || g[r][c] == '?' || g[r][c] == 'S' || g[r][c] == 'G') {
                if(vis[r][c][s] == 0) {
                    vis[r][c][s] = ops + 1;
                    q.push({r, c, s});
                }
...
// Problem Link: https://atcoder.jp/contests/abc420/tasks/abc420_d
