...
void build(vi& arr, int u, int l, int r) {
    if(l == r) {
        segTree[u] = {arr[l], max(0, arr[l])};
    } else {
        int m = (l + r) / 2;
        build(arr, u * 2, l, m);
        build(arr, u * 2 + 1, m + 1, r);

        segTree[u] = {segTree[u * 2].first + segTree[u * 2 + 1].first, max(segTree[u * 2].second, segTree[u * 2].first + segTree[u * 2 + 1].second)};
    }
    // cout << "BUILD: " << u << ' ' << l << ' ' << r << ' ' << segTree[u].first << ' '  << segTree[u].second << '\n'; 
}
...
void update(int u, int l, int r, int k, int x) {
    ...
    // cout << "Update: " << u << ' ' << l << ' ' << r << ' ' << segTree[u].first << ' '  << segTree[u].second << '\n'; 
}

pll query(int u, int l, int r, int tl, int tr) {
    if(tl > tr) return {0, 0};
    if(l == tl && r == tr) return segTree[u];
    int m = (l + r) / 2;
    pll left = query(u * 2, l, m, tl, min(m, tr));
    pll right = query(u * 2 + 1, m + 1, r, max(m + 1, tl), tr);
