...
void solve() {
    ...
    map<ll, vector<pll>> twoSum;
    rep(i, 0, n) {
        rep(j, i + 1, n) {
            twoSum[arr[i].ff + arr[j].ff].pb({i, j});
        }
    }
    ...
    rep(i, 0, n) {
        rep(j, i + 1, n) {
            ll target = x - arr[i].ff - arr[j].ff;
            
            for(auto [k, l]: twoSum[target]) {
                if(k != arr[i].ss && k != arr[j].ss && l != arr[i].ss && l != arr[j].ss) {
                    // Found
                    print(arr[i].ss + 1, arr[j].ss + 1, k + 1, l + 1);
                    return;
                }
            }
