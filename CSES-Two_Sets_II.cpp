...
void solve() {
    ...
    vvi dp(totalSum + 1, vi(n + 1, 0));

    if(totalSum & 1) {print(0); return;}

    rep(i, 0, n + 1) {
        dp[0][i] = 1;
    }

    rep(currSum, 1, totalSum + 1) {

        rep(currCoin, 1, (n + 1)) {
            if(currCoin > currSum) {
                dp[currSum][currCoin] = dp[currSum][currCoin - 1];
            }   
            else {
                dp[currSum][currCoin] = ( dp[currSum - currCoin][currCoin - 1] + dp[currSum][currCoin - 1] ) % MOD;
            }
        }

    }

    print(dp[(totalSum / 2) - n][n - 1]);
